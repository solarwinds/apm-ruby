  ...
  module OpenTelemetry
    ...
    class SolarWindsSampler
      ...
      def should_sample?(trace_id:, parent_context:, links:, name:, kind:, attributes:)
        ...
        SolarWindsAPM.logger.debug "####### should_sample? parameters \n
        ...
        SolarWindsAPM.logger.debug "####### sampling_result: #{sampling_result.inspect}"
      ...
      def calculate_liboboe_decision(parent_span_context, xtraceoptions, name, kind, attributes)
        ...
        if parent_span_context.valid? && parent_span_context.remote?
          ...
          SolarWindsAPM.logger.debug "####### calculate_liboboe_decision parent_span_context.remote? #{parent_span_context.remote?} with #{tracestring}"
        end

        SolarWindsAPM.logger.debug "####### name: #{name}, kind: #{kind}, attributes: #{attributes.inspect}"
        ...
        if tracing_mode.nil?
          SolarWindsAPM.logger.debug "####### transaction cache NOT found: #{transaction_naming_key}."
          ...
          SolarWindsAPM.logger.debug "####### transaction cache found: #{transaction_naming_key}."
        end
        ...
        SolarWindsAPM.logger.debug "####### decision parameters \n
      ...
      def create_xtraceoptions_response_value(decision, parent_span_context, xtraceoptions)
        SolarWindsAPM.logger.debug "####### create_xtraceoptions_response_value decision[auth]: #{decision['auth']}; decision[auth_msg]: #{decision['auth_msg']}; xtraceoptions.trigger_trace: #{xtraceoptions.trigger_trace}"
        ...
        if !decision["auth"] || decision["auth"] < 1
          ...
          SolarWindsAPM.logger.debug "####### create_xtraceoptions_response_value parent_span_context: #{parent_span_context}; tracestring: #{tracestring}; trigger_msg: #{trigger_msg}"
      ...
      def create_new_trace_state(parent_span_context, decision)
        ...
        SolarWindsAPM.logger.debug "Created new trace_state: #{trace_state.inspect}"
        trace_state
      end
      ...
      def calculate_trace_state(decision, parent_span_context, xtraceoptions)
        SolarWindsAPM.logger.debug "calculate_trace_state parent_span_context: #{parent_span_context.inspect}"
        if !parent_span_context.valid?
          ...
          if parent_trace_state.nil?
            ...
            SolarWindsAPM.logger.debug "Updated trace_state with span_id and sw trace_flags: #{trace_state.inspect}"
          end
        end
      ...
      def add_tracestate_capture_to_attributes_dict(attributes_dict, decision, trace_state, parent_span_context)
        ...
        SolarWindsAPM.logger.debug "####### tracestate_capture #{tracestate_capture.inspect}; attributes_dict #{attributes_dict.inspect}; trace_state #{trace_state.inspect}; parent_span_context: #{parent_span_context.inspect}"        
        ...
        SolarWindsAPM.logger.debug "####### trace_state_no_response #{trace_state_no_response.inspect}"
        ...
        SolarWindsAPM.logger.debug "####### attributes_dict #{attributes_dict.inspect}"
        attributes_dict
      end
      ...
      def calculate_attributes(attributes, decision, trace_state, parent_span_context, xtraceoptions)
        SolarWindsAPM.logger.debug "Received attributes: #{attributes.inspect}; decision:#{decision.inspect}; trace_state:#{trace_state.inspect}; parent_span_context:#{parent_span_context.inspect}; xtraceoptions:#{xtraceoptions.inspect}"

        otel_decision = otel_decision_from_liboboe(decision)
        return nil if Transformer.sampled?(otel_decision) == false
        
        SolarWindsAPM.logger.debug("Trace decision is_sampled - setting attributes #{otel_decision.inspect}")
        ...
        SolarWindsAPM.logger.debug "Set attributes with service entry internal KVs: #{new_attributes}"
        ...
        SolarWindsAPM.logger.debug "####### calculate_attributes sw_value: #{sw_value.inspect} parent_span_context.tracestate #{parent_span_context.tracestate.inspect}"
        ...
        if !parent_span_context.valid? || trace_state.nil?
          SolarWindsAPM.logger.debug "No valid traceparent or no tracestate - returning attributes: #{new_attributes}"
