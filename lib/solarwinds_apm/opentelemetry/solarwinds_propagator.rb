    ...
    module SolarWindsPropagator
      ...
      class TextMapPropagator
        ...
        def extract(carrier, context: ::OpenTelemetry::Context.current, getter: ::OpenTelemetry::Context::Propagation.text_map_getter)
          ...
          SolarWindsAPM.logger.debug "####### context(before): #{context.inspect} #{context.nil?}"
          ...
          SolarWindsAPM.logger.debug "####### xtraceoptions_header: #{xtraceoptions_header}"
          ...
          SolarWindsAPM.logger.debug "####### signature_header: #{signature_header}; propagator extract context: #{context.inspect}"
        ...
        def inject(carrier, context: ::OpenTelemetry::Context.current, setter: ::OpenTelemetry::Context::Propagation.text_map_setter)
          ...
          SolarWindsAPM.logger.debug "####### inject context: #{context.inspect}"
          
          cspan = ::OpenTelemetry::Trace.current_span(context)
          span_context = cspan&.context
          SolarWindsAPM.logger.debug "####### cspan #{cspan.inspect}; span_context #{span_context.inspect}"
          ...
          SolarWindsAPM.logger.debug "####### sw_value: #{sw_value}; trace_state_header: #{trace_state_header}"
          ...
          if trace_state_header.nil?
            ...
            SolarWindsAPM.logger.debug "####### creating new trace state: #{trace_state.inspect}"
          else
            ...
            SolarWindsAPM.logger.debug "Updating/Adding trace state for injection #{trace_state.inspect}"
          end
