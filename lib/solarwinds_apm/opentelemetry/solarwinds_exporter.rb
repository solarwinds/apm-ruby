  ...
  module OpenTelemetry
    ...
    class SolarWindsExporter
      ...
      def check_framework_version(framework)
        ...
        if @version_cache.keys.include? framework
          ...
          begin
            ...
            SolarWindsAPM.logger.debug "[#{self.class}/#{__method__}] couldn't load #{framework} with error #{e.message}; skip [#{self.class}/#{__method__}]#" 
          rescue StandardError => e
            SolarWindsAPM.logger.debug "[#{self.class}/#{__method__}] couldn't find #{framework} with error #{e.message}; skip [#{self.class}/#{__method__}]#" 
          ensure
      ...
      def add_info_transaction_name(span_data, evt)
        ...
        SolarWindsAPM.logger.debug "[#{self.class}/#{__method__}] txname #{txname} [#{self.class}/#{__method__}]#"
      ...
      def report_exception_event(span_event)
        ...
        SolarWindsAPM.logger.debug "[#{self.class}/#{__method__}] exception event #{evt.metadataString} [#{self.class}/#{__method__}]#"
      ...
      def report_info_event(span_event)
        ...
        SolarWindsAPM.logger.debug "[#{self.class}/#{__method__}] info event #{evt.metadataString} [#{self.class}/#{__method__}]#"
        @reporter.send_report(evt, with_system_timestamp: false)
      end
