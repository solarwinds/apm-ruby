...
module SolarWindsOTelAPM
  ...
  class TransactionSettings
    ...
    def initialize(url = '', headers = {}, options = nil)
      ...
      @sw_member_value = SolarWindsOTelAPM.trace_context.sw_member_value
      tracing_mode = AO_TRACING_ENABLED

      # TODO
      #   NH-11132 will address this
      # incoming tracing info has priority over existing context
      # if SolarWindsOTelAPM::Context.isValid && !@sw_member_value
      #   @do_sample = SolarWindsOTelAPM.tracing?
      #   return
      # end
      ...
      args << tracing_mode #2
      args << (SolarWindsOTelAPM::Config[:sample_rate] || OBOE_SETTINGS_UNSET) #3
      ...
      metrics, sample, @rate, @source, @bucket_rate, @bucket_cap, @type, @auth, @status_msg, @auth_msg, @status =
        SolarWindsOTelAPM::Context.getDecisions(*args)

      if @status > AO_TRACING_DECISIONS_OK
        SolarWindsOTelAPM.logger.warn "[solarwinds_apm/sample] Problem getting the sampling decisions: #{@status_msg} code: #{@status}"
      end
    ...
    def tracing_mode_disabled?
      SolarWindsOTelAPM::Config[:tracing_mode] &&
        [:disabled, :never].include?(SolarWindsOTelAPM::Config[:tracing_mode])
    end
    ...
    def tracing_enabled?(url)
      return false unless SolarWindsOTelAPM::Config[:url_enabled_regexps].is_a? Array
      # once we only support Ruby >= 2.4.0 use `match?` instead of `=~`
      return SolarWindsOTelAPM::Config[:url_enabled_regexps].any? { |regex| regex =~ url }
    rescue => e
      SolarWindsOTelAPM.logger.warn "[SolarWindsOTelAPM/filter] Could not apply :enabled filter to path. #{e.inspect}"
      true
    end
    ...
    def tracing_disabled?(url)
      return false unless SolarWindsOTelAPM::Config[:url_disabled_regexps].is_a? Array
      # once we only support Ruby >= 2.4.0 use `match?` instead of `=~`
      return SolarWindsOTelAPM::Config[:url_disabled_regexps].any? { |regex| regex =~ url }
    rescue => e
      SolarWindsOTelAPM.logger.warn "[SolarWindsOTelAPM/filter] Could not apply :disabled filter to path. #{e.inspect}"
      false
    end

    def trigger_tracing_mode_disabled?
      SolarWindsOTelAPM::Config[:trigger_tracing_mode] &&
        SolarWindsOTelAPM::Config[:trigger_tracing_mode] == :disabled
    end
    ...
    #
    def asset?(path)
      return false unless SolarWindsOTelAPM::Config[:dnt_compiled]
      # once we only support Ruby >= 2.4.0 use `match?` instead of `=~`
      return SolarWindsOTelAPM::Config[:dnt_compiled] =~ path
    rescue => e
      SolarWindsOTelAPM.logger.warn "[SolarWindsOTelAPM/filter] Could not apply do-not-trace filter to path. #{e.inspect}"
      false
    end
      ...
      def compile_url_settings_regexp(value)
        ...
        regexps.map! do |v|
          begin
            ...
            SolarWindsOTelAPM.logger.warn "[solarwinds_apm/config] Problem compiling transaction_settings item #{v}, will ignore."
            nil
          end
        end
