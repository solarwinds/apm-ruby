  ...
  class TransactionMetrics
    class << self
      ...
      def metrics(env, settings)
        if settings.do_metrics
          ...
          begin
            ...
            SolarWindsOTelAPM.transaction_name = send_metrics(env, req, url, start, status)
          rescue
            SolarWindsOTelAPM.transaction_name = send_metrics(env, req, url, start, status || '500')
            raise
          end
        else
          status, headers, response = yield
          SolarWindsOTelAPM.transaction_name = "#{domain(req)}#{transaction_name(env)}" if settings.do_sample
        end
      ...
      def send_metrics(env, req, url, start, status)
        ...
        # SolarWindsOTelAPM.logger.warn "%%% Sending metrics: #{name}, #{url}, #{status} %%%"
        SolarWindsOTelAPM::Span.createHttpSpan(name, url, domain(req), duration, status, method, error) || ''
      end

      def domain(req)
        if SolarWindsOTelAPM::Config['transaction_name']['prepend_domain']
          [80, 443].include?(req.port) ? req.host : "#{req.host}:#{req.port}"
        end
      end

      def transaction_name(env)
        return SolarWindsOTelAPM.transaction_name  if SolarWindsOTelAPM.transaction_name
